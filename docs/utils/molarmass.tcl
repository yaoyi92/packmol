#!/usr/bin/tclsh
#
# Computes the molar mass of some selection from a PDB file. 
# Considers CHARMM27 force field or default initial letter for
# each atom.
#
# Make it executable with: chmod +x maxmin.tcl
# Run with: ./molarmass.tcl
#
# L. Martinez, Apr 01, 2008. (leandromartinez98@gmail.com)
#
...
puts "-------------------------------------------------------"
puts " Computes the molar mass of a molecule. "
puts " The selection will be defined below: "
puts " Use 'all' to consider all atoms in each field. "
puts " Use 'ALL' to jump all question and consider everything. "
puts "-------------------------------------------------------"
...
if { [ string trim $segment ] == "ALL" } {
  set segment "all"
  set residue "all"
  set atomtype "all"
  set firstresidue "all"
  set lastresidue "all"
  set firstatom "all"
  set lastatom "all"
} else {
  ...
  puts -nonewline " Residue index greater or equal to: "; flush stdout; gets stdin firstresidue
  puts -nonewline " Residue index less or equal to: "; flush stdout; gets stdin lastresidue
  puts -nonewline " Atom index greater or equal to: "; flush stdout; gets stdin firstatom
  puts -nonewline " Atom index less or equal to: "; flush stdout; gets stdin lastatom
}
...
foreach line $file {
  if { [ string range $line 0 3 ] == "ATOM" |
    ...
    incr natoms
    set ss [ string trim [ string range $line 72 75 ] ]
    set rt [ string trim [ string range $line 17 20 ] ]
    set at [ string trim [ string range $line 12 15 ] ]
    set rr [ string trim [ string range $line 22 25 ] ]
    set consider true
    if { $firstresidue != "all" } { if { $rr < $firstresidue } { set consider false } }
    if { $lastresidue != "all" } { if { $rr > $lastresidue } { set consider false } }
    if { $firstatom != "all" } { if { $natoms < $firstatom } { set consider false } }
    if { $lastatom != "all" } { if { $natoms > $lastatom } { set consider false } }
...
puts "-------------------------------------------------------"
puts " Molar mass of the selected group: $molmass"
puts "-------------------------------------------------------"
